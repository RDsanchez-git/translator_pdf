...
def run_db_injection(target_docs: int, chunks_per_doc: int, db_path: str):

    absolute_db = os.path.abspath(db_path)

    print("=" * 70, flush=True)
    print("      SOTA CONTROL PLANE INJECTOR - BENCHMARK MODE", flush=True)
    print("=" * 70, flush=True)
    ...
    try:
        ...
        # Purga atómica del backlog previo para garantizar un benchmark limpio sin colisiones de clave única
        ...
        for i in range(target_docs):
            ...
            nodes = [
                f"node_{doc_id[-4:]}_{j}"
                for j in range(chunks_per_doc)
            ]

            task_repo.enqueue_tasks(
                doc_id,
                ast_hash,
                nodes
            )
    ...
    print(
        f"[OK] {injected_docs} documentos | "
        f"{injected_chunks} chunks inyectados "
        f"en {delta:.2f}s",
        flush=True
    )
