...
from core.chunking.protocols import NodeAtomicityPolicy, ChunkBoundaryPolicy
from core.validation.protocols import TokenEstimatorProtocol  # NUEVO import
...
class PolicyDrivenStreamingChunker:
    ...
    def __init__(
        self,
        estimator: TokenEstimatorProtocol,  # CAMBIADO: era TokenEstimator
    ...
    def chunk(self, stream: Iterator[ASTNode], max_tokens: int) -> Iterator[TranslationChunk]:
        ...
        for node in stream:
            ...
            node_tokens = self._estimator.estimate_tokens(node.text_content)
