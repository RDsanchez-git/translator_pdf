...
def test_duplicate_node_id_collision(validator):
    # SOTA FIX: Hidratación reglamentaria usando la factoría PayloadRegistry para cumplir las uniones estrictas de Pydantic v2
    nodes = [
        ASTNode(node_id="duplicated_id", node_type=ContentNodeType.PARAGRAPH, payload=PayloadRegistry.create(ContentNodeType.PARAGRAPH, "Prosa 1")),
        ASTNode(node_id="duplicated_id", node_type=ContentNodeType.PARAGRAPH, payload=PayloadRegistry.create(ContentNodeType.PARAGRAPH, "Prosa 2"))
    ]
...
def test_malformed_placeholder_syntax(validator):
    nodes = [
        ASTNode(node_id="node_1", node_type=ContentNodeType.PARAGRAPH, payload=PayloadRegistry.create(ContentNodeType.PARAGRAPH, "[[ASSET:TABLE:id_123"))
    ]
    warnings = validator.validate_ast(nodes)
